...
const goalTypesWithoutTarget = ["BOOLEAN", "ACTIVITIES", "QUALITATIVE"];
...
const Goals = () => {
  ...
  const { objectives, loadObjectives } = useObjectiveStore();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const payload = {
        ...form,
        targetValue: form.type === "BOOLEAN" ? true : form.targetValue,
      };
      ...
      setError(err.response?.data?.message || "Error creando la meta");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <MainLayout>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="pm-page-title mb-2">Metas</h1>
          <p className="text-sm text-slate-500">
            Convierte objetivos en resultados medibles.
          </p>
        </div>
        ...
        >
          {showCreateForm ? "Cerrar" : "+ Nueva meta"}
        </button>
      </div>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-500">
              Cada meta se conecta con sus actividades y también puede revisarse
              en Kanban.
            </p>
          </div>
